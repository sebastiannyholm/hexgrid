...
// players = {id: socket_id, name: username}
function createGame(players) {
	return makeBoard(players);
}

// Make all transactions "at the same time" => Two rounds:
// 		First round removes all resources being transfered from cells
// 		Second round transfers the amounts removed to their destination cells
function updateBoard(hexagons, playerTransactions) {
	if (isGameOver(hexagons)) return hexagons;
	const hexagonIdDict = getHexagonIdDict(hexagons);
	let validPlayerTransactions = [];
	// for (var i = 0; i < playerTransactions.length; i++) {
	for (const [playerId, transaction] of Object.entries(playerTransactions)) {
		try {
			validPlayerTransactions.push(validatePlayerTransaction(hexagons, playerId, transaction));
	...
	validPlayerTransactions.forEach(validPlayerTransaction => {
		// subtract resources being transferred
		hexagonIdDict[validPlayerTransaction.transaction.fromId].resources -= validPlayerTransaction.transaction.transferAmount;
	});

	validPlayerTransactions.forEach(validPlayerTransaction => {
		// add resources being transfered
		transferResources(validPlayerTransaction.playerId, hexagonIdDict[validPlayerTransaction.transaction.toId], validPlayerTransaction.transaction.transferAmount)
	});

	hexagons = growCells(hexagons);
	return hexagons;
}

function transferResources(playerId, toHexagon, amount) {
	...
	if (toHexagon.ownerId === playerId) {
		...
		if (toHexagon.resources < 0) {
			...
			toHexagon.ownerId = -1; // no one owns the hexagon
		}
	}
...
function validatePlayerTransaction(hexagons, playerId, transaction) {

	try {
		var fromHexagon = hexagons.find(hex => hex.id === transaction.fromId)
		var toHexagon = hexagons.find(hex => hex.id === transaction.toId)
		...
		if (fromHexagon.ownerId === toHexagon.ownerId && !ownedPathExists(hexagons, transaction.fromId, transaction.toId))
			throw "hexagons are not connected";
...
// run a DFS search for a path owned by the same player between the two hexagons
function ownedPathExists(hexagons, startHexagonId, endHexagonId) {
	const hexagonIdDict = getHexagonIdDict(hexagons);

	var stack = [];
	var explored = new Set();

	stack.push(startHexagonId);

	while (stack.length > 0) {
		var currentHexId = stack.pop();
		explored.add(currentHexId);

		if (currentHexId === endHexagonId)  	// search concluded, path exists
			return true;

		hexagonIdDict[currentHexId]
			.filter(neighborId => !explored.has(neighborId) && hexagonIdDict[neighborId].ownerId === hexagonIdDict[endHexagonId].ownerId)
			.forEach(neighborId => {
...
function getPlayerCells(hexagons, playerId) {
	// playerCell 	-> id, resources, maxGrowth, neighborCells
	// neighborCell -> id, resources, owner, maxGrowth

	var playerCells = [];
	for (var i = 0; i < hexagons.length; i++) {
		if (hexagons[i].ownerId === playerId) {
			var neighborCells = [];
			for (var j = 0; j < hexagons[i].neighbors.length; j++) {

				let neighborHex = hexagons.find(hex => hex.id === hexagons[i].neighbors[j]);

				var hexOwner = HexOwner.NONE;
				if (neighborHex.ownerId === playerId) hexOwner = HexOwner.OWN;
				else if (neighborHex.ownerId !== -1) hexOwner = HexOwner.OTHER;
...
// naive game over check..
function isGameOver(hexagons) {
	for (var i = 1; i < hexagons.length; i++) {
		if (hexagons[0].ownerId !== hexagons[i].ownerId) return false;
	}

	return hexagons[0].ownerId;
}
