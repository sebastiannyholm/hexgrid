...
class Hexgrid {
    ...
    performTransactions(validPlayerTransactions) {
        validPlayerTransactions.forEach(validPlayerTransaction => {
            ...
            this.hexagonIdDict[validPlayerTransaction.transaction.fromId].resources -= validPlayerTransaction.transaction.transferAmount;
        });

        validPlayerTransactions.forEach(validPlayerTransaction => {
            // add resources being transfered
            this._addResources(validPlayerTransaction.playerId, validPlayerTransaction.transaction.toId, validPlayerTransaction.transaction.transferAmount)
        });
    }
    ...
    _ownedPathExists(startHexId, endHexId) {
        ...
        while (stack.length > 0) {
            ...
            if (currentHexId === endHexId)  	// search concluded, path exists
                return true;

            this.hexagonIdDict[currentHexId].neighbors
                .filter(neighborId => !explored.has(neighborId) && this.hexagonIdDict[neighborId].ownerId === this.hexagonIdDict[endHexId].ownerId)
                .forEach(neighborId => {
