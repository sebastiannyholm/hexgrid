...
function turn(myCells) {
  ...
  function hasFreeNones(cells) {
    for (var i = 0; i < cells.length; i++) {
      for (var j = 0; j < cells[i].neighbors.length; j++) {
        if (cells[i].neighbors[j].owner === HexOwner.NONE && cells[i].neighbors[j].resources === 0) return true;
      }
    }
    return false;
  }
  ...
  for (var i = 0; i < maxCluster.length; i++) {
    ...
    for (var j = 0; j < cell.neighbors.length; j++) {
      ...
      if (freeNones) {
        ...
        // this approach depends solely on foritifaction cells
        ...
        // find cells bordering free none, expanding the bloom
  ...
  if (!freeNones && target.owner !== HexOwner.OWN && myCells.length !== 1) {
    ...
    let totalResourcesNeededForTakeover =
      sourceResourcesAfterTakeover + targetResourcesAfterTakeover + target.resources;
    ...
    if (resourcesNeededForTakeover < 0) {
      // can perform takeover
      ...
      return {
        fromId: bestHelper.id,
        toId: source.id,
        transferAmount: bestSupply,
      };
    }
  }
