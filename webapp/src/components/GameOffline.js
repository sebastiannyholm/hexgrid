...
function GameOffline(props) {
    ...
    const gameSidebarElements = [
        <span key={0} className="sidebar-item interactable" onClick={onPlay}>Play</span>,
        <span key={1} className="sidebar-item interactable" onClick={onStop}>Stop</span>,
        <span key={2} className="sidebar-item interactable" onClick={onNextStep}>Next step</span>,
        ...
        <span key={6} className="sidebar-item interactable" onClick={onRestart}>Restart</span>,
    ...
    return (
        <WithSidebar
            ...
            content={gameState ? <div className="game" onClick={winner ? () => setWinner(null) : null}>
                <HexgridComponent hexagons={gameState.hexagons} players={gameState.players} orientation={hexgridOrientation} />
            ...
            </div> : <Loader>Loading game...</Loader>}
        />
    );
}
