...
function Editor(props) {
    ...
    const JsAceEditor = <AceEditor className="editor"
        width="800"
        height="1000"
        placeholder=""
        mode="javascript"
    ...
    const editorSidebarElements = [
        <span key={0} id="input-player-name" className="sidebar-item interactable single-line" contentEditable suppressContentEditableWarning={true} placeholder="enter algorithm name.." spellCheck={false} onInput={e => setPlayerName(e.target.textContent)} />,
        <hr key={1}/>,
        <span key={2} className="sidebar-item interactable" onClick={onSave}>Save</span>,
        <span key={3} className="sidebar-item interactable" onClick={onCompile}>Compile</span>,
    ];
    ...
    const Content = () => {
        ...
        return <WithSidebar
                    sidebarElements={editorSidebarElements}
                    content={JsAceEditor}
                />
    }    
