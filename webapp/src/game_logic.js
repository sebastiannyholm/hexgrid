...
function setupGame(codes) {
  ...
  gameSteps.push({
    round: rounds,
    hexagons: gameStep,
    playerStandings: getPlayerStandings(),
  });
}
  ...
  sortedPlayers.sort(function (p1, p2) {
    if (p1.hexagonCount === p2.hexagonCount) {
      if (p1.resources === p2.resources) {
        if (p1.roundsSurvived === p2.roundsSurvived) return p2.exceptions - p1.exceptions; // sort by number of exceptions
...
export function restartGame(codes) {
...
function updateBoard(validTransactions) {
  ...
  for (var i = 0; i < validTransactions.length; i++) {
    hexagonIdDict[validTransactions[i].transaction.fromId].resources -=
...
function ownedPathExists(startHex, endHex) {
  ...
  while (stack.length > 0) {
    ...
    hexNeighbors(currentHex)
      .filter((n) => !explored.has(n) && n.ownerId === endHex.ownerId)
      .forEach((n) => {
        stack.push(n);
      });
  }
...
function validateTransaction(player, transaction) {
  try {
    ...
    else if (fromHexagon.ownerId !== player.id) throw 'cannot transfer resources from cells you do not own';
    else if (fromHexagon === toHexagon) throw 'cannot transfer to same cell';
...
function transferResources(player, hexToId, transferAmount) {
  ...
  if (toHexagon.ownerId === player.id) toHexagon.resources += transferAmount;
  else {
...
function growCells() {
  for (var i = 0; i < hexagons.length; i++) {
    if (hexagons[i].ownerId !== -1 && hexagons[i].resources < hexagons[i].maxGrowth) hexagons[i].resources++;
  }
}
    ...
    for (var c = 0; c < cubeNeighbors.length; c++) {
      ...
      for (var j = 0; j < hexagons.length; j++) {
        if (cubeEqual(hexagons[j].cube, neighborCube)) {
          ...
          if (hexagon.neighbors == null) hexagon.neighbors = [neighborHex.id];
          else hexagon.neighbors.push(neighborHex.id);
