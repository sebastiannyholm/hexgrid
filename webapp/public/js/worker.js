// import {restartGame} from './game_logic.js';
...
onmessage = function (e) {
	...
	const gameSteps = restartGame(e.data)
	console.log(gameSteps)
	processGameResult(gameSteps);
}

function processGameResult(gameSteps) {
	gameSteps.forEach(step => {
		for (var i = 0; i < step.playerStandings.length; i++) {
			step.playerStandings[i] = {
				color: step.playerStandings[i].hexagonCount !== 0 ? PLAYER_COLORS[step.playerStandings[i].id] : { bg: "#808080", fg: "#404040" },
				data: {
					name: step.playerStandings[i].name,
					hexagonCount: step.playerStandings[i].hexagonCount,
					resources: step.playerStandings[i].resources,
					roundsSurvived: step.playerStandings[i].roundsSurvived,
					exceptions: step.playerStandings[i].exceptions
				}
			}
		}
	});
	postMessage(gameSteps);
}
...
function transferResources(player, hexToId, transferAmount) {
	...
	if (toHexagon.ownerId === player.id)
		...
		if (toHexagon.resources < 0) {
			...
			toHexagon.ownerId = HexOwner.NONE;
		}
	}
}
...
function growCells() {
	for (var i = 0; i < hexagons.length; i++) {
		if (hexagons[i].ownerId !== HexOwner.NONE && hexagons[i].resources < hexagons[i].maxGrowth)
	...
	for (var i = 0; i < hexagons.length; i++) {
		if (hexagons[i].ownerId === player.id) {
			...
			for (var j = 0; j < hexagons[i].neighbors.length; j++) {
				...
				else if (neighborHex.ownerId !== HexOwner.NONE) hexOwner = HexOwner.OTHER;
