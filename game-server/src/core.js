import {List, Map, fromJS, toJS, toJSON} from 'immutable';
// import {makeBoard} from './game_logic';

export function createGame(playerNames) {
	const players = playerNames.map((name, index) => ({id: index, name: name}));
	const gameState = Map({
		players: players,
		hexagons: makeBoard(players.toJS())
	});
...
export function nextStep(state, transactions) {
